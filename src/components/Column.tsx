    ...
    <div className="flex flex-col h-full min-w-[300px] max-w-[350px]">
      <div
        className={`flex items-center justify-between p-4 rounded-t-lg border-2 ${
          isOver ? 'border-primary-500 dark:border-primary-400 bg-primary-50 dark:bg-primary-900/20' : config.color
        } transition-colors`}
      >
        <div className="flex items-center gap-2">
          <h2 className="font-bold text-gray-800 dark:text-gray-200">{column.title}</h2>
          <span className="bg-white dark:bg-gray-800 text-gray-600 dark:text-gray-300 text-xs font-semibold px-2 py-1 rounded-full">
            {column.tasks.length}
          </span>
        </div>
        <button
      ...
      <div
        ...
        className={`flex-1 p-4 space-y-3 overflow-y-auto rounded-b-lg ${
          isOver ? 'bg-primary-50 dark:bg-primary-900/20' : 'bg-gray-50 dark:bg-gray-900/50'
